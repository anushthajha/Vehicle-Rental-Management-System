...
def _bank_payload(profile: HostProfile | None) -> dict:
...
async def _get_or_create_profile(db: AsyncSession, host_id: str) -> HostProfile:
    profile = await db.scalar(select(HostProfile).where(HostProfile.user_id == host_id))
    if profile is None:
        profile = HostProfile(user_id=host_id)
        db.add(profile)
        await db.flush()
    return profile


async def _primary_image_map(db: AsyncSession, car_ids: list[str]) -> dict[str, str | None]:
    if not car_ids:
        return {}
    images = (
        await db.execute(
            ...
            .order_by(CarImage.car_id, CarImage.is_primary.desc(), CarImage.order_index.asc())
        )
    ).scalars().all()
    result: dict[str, str | None] = {}
    for image in images:
        result.setdefault(image.car_id, image.image_url)
    return result


async def _host_wallet_credits(db: AsyncSession, host_id: str, start: datetime | None = None, end: datetime | None = None) -> Decimal:
    conditions = [WalletTransaction.user_id == host_id, WalletTransaction.transaction_type == "credit"]
    if start:
...
async def _summary_data(db: AsyncSession, host: User) -> dict:
    now = datetime.utcnow()
    ...
    wallet = await get_or_create_wallet(db, host.id)
    total = await _host_wallet_credits(db, host.id)
...
async def monthly_earnings(
    ...
    rows = (
        await db.execute(
            ...
            .where(Booking.host_id == current_user.id, Booking.status == "completed", func.year(Booking.actual_return_time) == year)
    ...
    return [
        {
            "car_id": car_id,
            "title": title,
            "primary_image": images.get(car_id),
            "trips": trips,
            "gross": money(gross),
        ...
        for car_id, title, rating, trips, gross, platform_fees, net in rows
    ]
...
async def request_payout(payload: PayoutRequest, current_user: User = Depends(require_host), db: AsyncSession = Depends(get_db)):
    ...
    pending = await db.scalar(
        select(HostPayoutRequest).where(HostPayoutRequest.host_id == current_user.id, HostPayoutRequest.status.in_(("pending", "processing")))
    )
    if pending:
    ...
    for admin in admins:
        await create_notification(admin.id, "Payout request", f"{current_user.full_name} requested ₹{money(amount):,.2f}.", "host", action_url="/admin/payouts", meta={"payout_id": payout.id})
    await db.commit()
...
async def update_host_profile(payload: HostProfileUpdateRequest, current_user: User = Depends(require_host), db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    return {"message": "Host profile updated"}
...
async def host_profile(current_user: User = Depends(require_host), db: AsyncSession = Depends(get_db)):
    ...
    cancellation_count = await db.scalar(
        select(func.count())
        .select_from(Booking)
        .where(Booking.host_id == current_user.id, Booking.status == "cancelled", Booking.cancelled_by == current_user.id)
    ) or 0
    return {
        "profile": {
            "id": profile.id,
            ...
            "joined_as_host_at": _dt(profile.joined_as_host_at),
            "bank_account": _bank_payload(profile),
        },
        "stats": summary | {
            "host_cancellations": cancellation_count,
            "superhost_criteria": {
