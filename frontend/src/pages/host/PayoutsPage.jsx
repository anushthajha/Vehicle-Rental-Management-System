...
export default function PayoutsPage() {
  ...
  return <div className="px-4 py-8"><section className="mx-auto max-w-6xl space-y-6"><div><p className="text-sm font-black uppercase text-zoomcar">Host Payouts</p><h1 className="text-3xl font-black">Withdraw earnings</h1></div>
    ...
    <section className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm"><h2 className="font-black">Request Payout</h2><div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]"><input type="number" min="500" className="input" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Amount, min ₹500" /><button disabled={disabled} onClick={() => setConfirm(true)} className="rounded-md bg-zoomcar px-5 py-3 font-black text-white disabled:opacity-50">Request Payout</button></div><p className="mt-2 text-sm font-bold text-zinc-500">{bank.has_bank_account ? `Available balance ${money(summary.wallet_balance)}` : 'Add bank account first'}</p></section>
    ...
    {confirm && <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4"><div className="w-full max-w-sm rounded-lg bg-white p-5"><h3 className="text-lg font-black">Request payout of {money(amount)}?</h3><p className="mt-2 text-sm font-bold text-zinc-500">This amount will be held from your wallet immediately.</p><div className="mt-5 flex justify-end gap-2"><button onClick={() => setConfirm(false)} className="rounded-md border border-zinc-200 px-4 py-2 font-black">Cancel</button><button onClick={request} className="rounded-md bg-zoomcar px-4 py-2 font-black text-white">Confirm</button></div></div></div>}
  </section></div>
}
