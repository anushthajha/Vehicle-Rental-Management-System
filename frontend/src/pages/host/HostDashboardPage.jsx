...
export default function HostDashboardPage() {
  ...
  async function load() {
    const [summaryData, profileData, monthlyData, carsData, bookingsData] = await Promise.all([
      ...
      api.get('/cars/host/my-cars'),
      api.get('/bookings/', { params: { as_role: 'host', limit: 5 } }),
    ])
  ...
  if (!summary) return <div className="grid min-h-screen place-items-center"><div className="h-10 w-10 animate-spin rounded-full border-4 border-zinc-200 border-t-zoomcar" /></div>

  return <div className="px-4 py-8"><section className="mx-auto max-w-7xl space-y-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-black uppercase text-zoomcar">Host Dashboard</p><h1 className="text-3xl font-black">Your hosting cockpit</h1></div><div className="flex gap-2"><Link to="/host/list-car" className="inline-flex items-center gap-2 rounded-md bg-zoomcar px-4 py-3 font-black text-white"><Plus size={18} /> List New Car</Link><Link to="/host/payouts" className="rounded-md bg-zinc-950 px-4 py-3 font-black text-white">Withdraw Earnings</Link></div></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_1.2fr]"><section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h2 className="font-black">Active Listings</h2><Link to="/host/my-cars" className="text-sm font-black text-zoomcar">View All</Link></div><div className="mt-4 grid gap-3">{cars.slice(0, 3).map((car) => <div key={car.id} className="flex items-center gap-3 rounded-md border border-zinc-200 p-3"><img alt="" src={car.primary_image_url || '/vite.svg'} className="h-16 w-20 rounded object-cover" /><div className="min-w-0 flex-1"><p className="truncate font-black">{car.title}</p><p className="text-sm font-bold text-zinc-500">{car.average_rating} ★ · {car.total_trips} trips</p></div><span className={`rounded-full px-2 py-1 text-xs font-black ${car.is_available && car.is_approved ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{car.is_available && car.is_approved ? 'Active' : 'Review'}</span></div>)}</div></section><section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="font-black">Recent Bookings</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[650px] text-left text-sm"><thead className="text-xs uppercase text-zinc-500"><tr><th className="py-2">Guest</th><th>Car</th><th>Dates</th><th>Amount</th><th>Status</th><th>Action</th></tr></thead><tbody>{bookings.map((booking) => <tr key={booking.id} className="border-t border-zinc-100"><td className="py-3 font-bold">{booking.counterparty?.name}</td><td>{booking.car?.title}</td><td>{formatDateTime(booking.pickup_datetime)}</td><td className="font-black">{moneyLabel(booking.host_earnings)}</td><td><span className={`rounded-full px-2 py-1 text-xs font-black capitalize ${statusClass(booking.status)}`}>{booking.status}</span></td><td><Link to="/host/bookings" className="font-black text-zoomcar">Open</Link></td></tr>)}</tbody></table></div></section></div>
  </section></div>
}

function Stat({ icon: Icon, label, value, danger }) {
  return <div className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm"><Icon className={danger ? 'text-zoomcar' : 'text-zinc-900'} size={23} /><p className="mt-4 text-sm font-bold text-zinc-500">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div>
}
