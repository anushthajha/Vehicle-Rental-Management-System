  ...
  return (
    <main className="min-h-screen bg-zinc-50 px-4 py-8">
      <section className="mx-auto max-w-5xl">
        <p className="text-sm font-black uppercase text-sigfleet">Host</p>
        <h1 className="text-3xl font-black text-zinc-950">Active Trips</h1>
        {loading ? <div className="grid h-64 place-items-center"><Loader2 className="animate-spin text-sigfleet" /></div> : <div className="mt-5 grid gap-4">{trips.map((trip) => <HostBookingCard key={trip.id} booking={trip} onEnd={setEnding} />)}{!trips.length && <div className="rounded-lg border border-dashed border-zinc-300 bg-white p-12 text-center font-black text-zinc-500">No active trips.</div>}</div>}
      </section>
