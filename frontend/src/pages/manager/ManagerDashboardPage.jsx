...
import { CalendarDays, Car as Vehicle, CheckCircle2, ChevronRight, Clock, IndianRupee, Plus, Route, Star, Trophy } from 'lucide-react'
...
export default function ManagerDashboardPage() {
  ...
  async function load() {
    const [summaryData, profileData, monthlyData, carsData, bookingsData] = await Promise.all([
      getManager('/stats').catch(() => null),
      getManager('/profile').catch(() => null),
      getManager('/earnings/monthly', { year: new Date().getFullYear() }).catch(() => []),
      api.get('/vehicles/manager/my-vehicles').catch(() => ({ data: { vehicles: [] } })),
      api.get('/bookings/', { params: { as_role: 'vehicle_manager', limit: 5 } }).catch(() => ({ data: { bookings: [] } })),
    ])
    setSummary(summaryData || { total_vehicles: 0, active_vehicles: 0, pending_requests: 0, active_rentals: 0, this_month_revenue: 0, acceptance_rate: 0, monthly_bookings: [] })
    setProfile(profileData)
    setMonthly((monthlyData || []).slice(-6))
    setCars(carsData.data?.vehicles || carsData.vehicles || [])
    setBookings(bookingsData.data?.bookings || bookingsData.bookings || [])
  }
  ...
  return <div className="px-4 py-8"><section className="mx-auto max-w-7xl space-y-6"><DailyBriefBanner /><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-black uppercase text-sigfleet">Manager Dashboard</p><h1 className="text-3xl font-black">Welcome back, {user?.full_name || profile?.profile?.user?.full_name || 'Manager'}</h1><span className="mt-2 inline-flex rounded-full bg-[#1E3A5F] px-3 py-1 text-xs font-black text-white">Vehicle Manager</span></div><div className="flex gap-2"><Link to="/manager/vehicles/add" className="inline-flex items-center gap-2 rounded-md bg-sigfleet px-4 py-3 font-black text-white"><Plus size={18} /> Add New Vehicle</Link><Link to="/manager/payouts" className="rounded-md bg-zinc-950 px-4 py-3 font-black text-white">Withdraw Earnings</Link></div></div>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><Stat icon={Vehicle} label="My Vehicles" value={<Count value={summary.total_vehicles} />} sub={`Active: ${summary.active_vehicles || 0} | Inactive: ${inactiveVehicles}`} to="/manager/vehicles" /><Stat icon={Clock} label="Active Bookings" value={summary.pending_requests} sub="Awaiting your approval" danger={summary.pending_requests > 0} to="/manager/bookings" /><Stat icon={IndianRupee} label="Total Earnings" value={money(summary.this_month_revenue)} sub="Net after platform fee" to="/manager/earnings" /><Stat icon={Route} label="Active Trips" value={summary.active_rentals} sub="Currently on the road" tone="text-emerald-600" to="/manager/trips/active" /><Stat icon={CheckCircle2} label="Pending Inspections" value={summary.active_rentals} sub="Inspect active returns" tone={acceptanceTone} to="/manager/bookings?tab=active" /></div>
    {profile?.profile?.is_super_manager && <div className="rounded-lg bg-gradient-to-r from-amber-400 to-yellow-600 p-5 text-white shadow-sm"><div className="flex items-center gap-3"><Trophy size={28} /><div><h2 className="text-xl font-black">You're a Super Manager!</h2><p className="font-bold">Criteria: {profile?.stats?.super_manager_criteria?.completed_trips || 0} trips, {profile?.stats?.super_manager_criteria?.average_rating || 0} rating, {profile?.stats?.super_manager_criteria?.acceptance_rate || 0}% acceptance, {profile?.stats?.super_manager_criteria?.manager_cancellations_90d || 0} recent manager cancellations.</p></div></div></div>}
...
function Stat({ icon: Icon, label, value, sub, danger, tone = 'text-zinc-900', to }) {
  const body = <><Icon className={danger ? 'text-sigfleet' : tone} size={23} /><p className="mt-4 text-sm font-bold text-zinc-500">{label}</p><p className="mt-1 text-2xl font-black">{value}</p>{sub && <p className="mt-1 text-xs font-bold text-zinc-500">{sub}</p>}{to && <ChevronRight className="absolute bottom-4 right-4 text-zinc-300" size={18} />}</>
  return to ? <Link to={to} className="relative cursor-pointer rounded-lg border border-zinc-200 bg-white p-5 shadow-sm transition-all duration-200 hover:scale-[1.02] hover:shadow-lg">{body}</Link> : <div className="relative rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">{body}</div>
}
