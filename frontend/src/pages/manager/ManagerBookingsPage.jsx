  ...
  return (
    <main className="min-h-screen bg-zinc-50 px-4 py-8">
      <section className="mx-auto max-w-6xl">
        ...
        {loading ? <div className="grid h-64 place-items-center"><Loader2 className="animate-spin text-sigfleet" /></div> : <div className="mt-5 grid gap-4">{bookings.map((booking) => <ManagerBookingCard key={booking.id} booking={booking} onAccept={accept} onReject={setRejecting} onStart={start} onEnd={setEnding} />)}{!bookings.length && <div className="rounded-lg border border-dashed border-zinc-300 bg-white p-12 text-center font-black text-zinc-500">No bookings in this view.</div>}</div>}
      </section>
...
export function ManagerBookingCard({ booking, onAccept, onReject, onStart, onEnd }) {
  ...
  return <article className={`rounded-lg border bg-white p-4 shadow-sm ${booking.status === 'pending' ? 'border-amber-300' : 'border-zinc-200'}`}><div className="grid gap-4 lg:grid-cols-[1fr_auto]"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-black text-zinc-950">{booking.car?.title}</h2><span className={`rounded-full px-2 py-1 text-xs font-black capitalize ${statusClass(booking.status)}`}>{booking.status}</span>{booking.status === 'pending' && <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-black text-emerald-700">verified guest</span>}</div><p className="mt-2 font-bold text-zinc-500">Guest: {booking.counterparty?.name || 'Guest'}</p><p className="font-bold text-zinc-500">{formatDateTime(booking.pickup_datetime)} - {formatDateTime(booking.return_datetime)} · {bookingDuration(booking)}</p><p className="mt-2 text-lg font-black text-emerald-700">Manager earnings {moneyLabel(booking.manager_earnings)}</p>{booking.status === 'pending' && <p className="mt-1 flex items-center gap-1 text-sm font-bold text-amber-700"><Clock size={15} /> Expires in: {expiresAt} hours</p>}{booking.status === 'active' && <p className="mt-1 text-sm font-black text-blue-700">Trip in progress</p>}</div><div className="flex flex-wrap items-center gap-2">{booking.status === 'pending' && <><button onClick={() => onAccept(booking.id)} className="inline-flex items-center gap-2 rounded-md bg-emerald-600 px-4 py-2 font-black text-white"><CheckCircle2 size={17} /> Accept</button><button onClick={() => onReject(booking)} className="inline-flex items-center gap-2 rounded-md bg-red-600 px-4 py-2 font-black text-white"><XCircle size={17} /> Reject</button></>}{canStart && <button onClick={() => onStart(booking.id)} className="inline-flex items-center gap-2 rounded-md bg-zinc-950 px-4 py-2 font-black text-white"><Play size={17} /> Start Trip</button>}{booking.status === 'active' && <button onClick={() => onEnd(booking)} className="inline-flex items-center gap-2 rounded-md bg-sigfleet px-4 py-2 font-black text-white"><Square size={17} /> End Trip</button>}<Link to={`/dashboard/bookings/${booking.id}`} className="rounded-md border border-zinc-300 px-4 py-2 font-black text-zinc-800">Details</Link></div></div></article>
}
