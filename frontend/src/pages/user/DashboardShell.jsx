  ...
  return (
    <main className="min-h-screen bg-zinc-50 text-zinc-950 flex">
      ...
      <section className="flex-1 min-w-0 lg:pl-64 flex flex-col min-h-screen">
        {/* Unified Top Header for Page Title, Notification Bell, and Page Specific Actions */}
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-zinc-200 bg-white px-4 lg:px-8 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              {eyebrow && <p className="text-[10px] font-black uppercase tracking-wider text-[#E31837]">{eyebrow}</p>}
              {title && <h1 className="text-lg font-black text-zinc-900 leading-tight">{title}</h1>}
            </div>
          </div>
          <div className="flex items-center gap-3">
            <NotificationBell />
        ...
        {/* Dashboard Page Main Content Slot */}
        <div className="flex-1 p-4 lg:p-8 overflow-y-auto">
          {children}
        </div>
      </section>

      {/* Mobile Drawer Navigation (radix-ui) */}
      <Dialog.Root open={mobileOpen} onOpenChange={setMobileOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed inset-y-0 left-0 z-50 w-72 bg-zinc-950 text-white lg:hidden shadow-2xl transition duration-300">
            <Dialog.Title className="sr-only">Dashboard Navigation Menu</Dialog.Title>
            <button
