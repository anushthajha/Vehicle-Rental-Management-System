...
export function AdminBookingsPage() {
  ...
  return <DataShell title="Bookings" subtitle="Trip operations across customers, managers, cities, and dates."><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-zinc-50 text-xs uppercase text-zinc-500"><tr><th className="p-4">Ref</th><th className="p-4">Vehicle</th><th className="p-4">Customer</th><th className="p-4">City</th><th className="p-4">Pickup</th><th className="p-4">Return</th><th className="p-4">Amount</th><th className="p-4">Status</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t border-zinc-100"><td className="p-4 font-black">{row.booking_ref}</td><td className="p-4">{row.vehicle_name}</td><td className="p-4">{row.customer_name}</td><td className="p-4">{row.city}</td><td className="p-4">{formatDate(row.pickup_datetime)}</td><td className="p-4">{formatDate(row.return_datetime)}</td><td className="p-4 font-black">{formatMoney(row.total_amount)}</td><td className="p-4"><StatusBadge value={row.status} /></td></tr>)}</tbody></table></DataShell>
}
