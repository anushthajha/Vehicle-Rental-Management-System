...
export function AdminPayoutsPage() {
  ...
  return <DataShell title="Payouts" subtitle="Process host payout requests and mark paid or failed."><table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-zinc-50 text-xs uppercase text-zinc-500"><tr><th className="p-4">Host</th><th className="p-4">Amount</th><th className="p-4">Requested</th><th className="p-4">Processed</th><th className="p-4">Status</th><th className="p-4">Actions</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t border-zinc-100"><td className="p-4 font-black">{row.host.name}<p className="text-xs font-bold text-zinc-500">{row.host.email}</p></td><td className="p-4 font-black">{formatMoney(row.amount)}</td><td className="p-4">{formatDate(row.requested_at)}</td><td className="p-4">{formatDate(row.processed_at)}</td><td className="p-4"><StatusBadge value={row.status} /></td><td className="p-4"><div className="flex gap-2"><button onClick={() => action(row, 'process')} className="rounded-md border border-zinc-200 px-3 py-2 text-xs font-black">Process</button><button onClick={() => action(row, 'complete')} className="rounded-md bg-emerald-600 px-3 py-2 text-xs font-black text-white">Complete</button><button onClick={() => action(row, 'fail')} className="rounded-md bg-[#E31837] px-3 py-2 text-xs font-black text-white">Fail</button></div></td></tr>)}</tbody></table></DataShell>
}
