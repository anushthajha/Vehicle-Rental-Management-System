...
export default function AdminSupportPage() {
  ...
  const load = () => getAdmin('/support', { status, limit: 50 }).then((data) => {
    setTickets(data.items || [])
    setActive((current) => current ? (data.items || []).find((item) => item.id === current.id) || data.items?.[0] : data.items?.[0])
  })
