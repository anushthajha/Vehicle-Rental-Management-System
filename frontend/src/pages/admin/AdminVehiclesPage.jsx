...
import { formatDate, formatMoney, getAdmin, patchAdmin, putAdmin } from './adminApi'
import { useVehicleCategories } from '../../hooks/useVehicleCategories'

const tabs = ['pending', 'approved', 'rejected', 'all']

export default function AdminVehiclesPage({ initialTab = 'pending' }) {
  ...
  const load = () => getAdmin('/vehicles', { status: tab, ...filters, city: filters.city || undefined, category_id: filters.category || undefined, limit: 50 })
  ...
  const approve = async (car) => {
    try {
      await putAdmin(`/vehicles/${car.id}/approve`)
      toast.success('Vehicle approved. Manager has been notified.')
      setCars((current) => current.map((item) => item.id === car.id ? { ...item, status: 'approved', is_approved: true, is_available: true } : item))
      setActiveCar(null)
    } catch (err) {
      toast.error(err.message || 'Failed to approve vehicle')
    }
  }
...
function Badge({ value }) {
  const pending = ['pending', 'pending_approval'].includes(value)
  const styles = value === 'approved' ? 'bg-emerald-50 text-emerald-700' : pending ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'
...
function RejectModal({ car, onClose, onDone }) {
  ...
  const save = async () => {
    try {
      await putAdmin(`/vehicles/${car.id}/reject`, { reason })
      toast.success('Vehicle rejected. Manager has been notified.')
      onDone()
      onClose()
    } catch (err) {
      toast.error(err.message || 'Failed to reject vehicle')
    }
  }
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4"><div className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl"><h3 className="text-lg font-black">Reject {car.title}</h3><textarea className="input mt-4 min-h-32" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason for manager (required)" /><div className="mt-4 flex justify-end gap-2"><button onClick={onClose} className="rounded-md border border-zinc-200 px-4 py-2 font-black">Cancel</button><button disabled={reason.length < 3} onClick={save} className="rounded-md bg-[#E31837] px-4 py-2 font-black text-white disabled:opacity-50">Reject</button></div></div></div>
}
