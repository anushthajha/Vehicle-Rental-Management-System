...
export default function HomePage() {
  ...
  useEffect(() => {
    api.get('/cars/featured', { params: { limit: 6 } }).then((response) => setFeatured(response.data.cars || response.data || [])).catch(() => setFeatured([])).finally(() => setLoading(false))
  }, [])

  const estimated = Math.round(days * rates[category] * 0.85)
  const search = () => navigate(`/search?city=${encodeURIComponent(city)}${pickup ? `&pickup=${encodeURIComponent(pickup)}` : ''}${dropoff ? `&return=${encodeURIComponent(dropoff)}` : ''}`)

  return (
    <main className="bg-[#F9FAFB] text-[#111827]">
      <Navbar />
      <section className="relative min-h-[92vh] overflow-hidden bg-[#111827]">
        <img src="https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?w=1920" alt="" className="absolute inset-0 h-full w-full object-cover" />
...
function Testimonials() {
  const items = [
    ['Booked a Creta for our Coorg trip — host was amazing and car was spotless.', 'Radhika M., Bengaluru'],
    ['Easy booking, clean car, smooth pickup. Way better than cabs for road trips!', 'Aryan K., Mumbai'],
    ['The Thar we rented for Leh was perfect. Highly recommend Zoomcar!', 'Priya S., Delhi'],
  ]
...
function FAQ() {
  const questions = [
    ['How does the booking process work?', 'Search by city and dates, choose a verified car, complete your profile and KYC, then pay securely. The host confirms the trip and shares pickup instructions.'],
    ...
    ['Can I extend my trip while on the go?', 'Yes. Open the active booking, choose a new return time, and wait for host approval. Any extra amount is collected before confirmation.'],
    ['Is insurance mandatory?', 'Yes. Every booking includes an insurance selection so guests and hosts know the damage-liability limit before pickup.'],
    ['How do hosts get paid?', 'Host earnings are credited after successful trip completion. Hosts can request payouts to a verified bank account from the host dashboard.'],
  ]
...
const guestSteps = [
  ...
  { title: 'Pick Up Your Car', text: 'Meet your host, complete a quick inspection, and get the keys.' },
  { title: 'Drive & Return', text: "Drive anywhere you want, return on time, and you're done." },
]
const hostSteps = [
  { title: 'List Your Car Free', text: 'Add your car in 10 minutes. No subscription fees ever.' },
...
const trust = [
  ...
  { icon: Star, title: '24/7 Support', text: 'Round-the-clock assistance for guests and hosts.' },
