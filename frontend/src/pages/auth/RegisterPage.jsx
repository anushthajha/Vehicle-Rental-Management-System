  ...
  return (
    <AuthLayout>
      <form onSubmit={submit} className="w-full max-w-md rounded-lg border border-zinc-200 bg-white p-8 shadow-sm">
        ...
        <label className="mt-6 block text-sm font-bold text-zinc-800">Full Name<input name="full_name" value={form.full_name} onChange={update} required className="mt-2 w-full rounded-md border-zinc-300" /></label>
        <label className="mt-4 block text-sm font-bold text-zinc-800">Email<input name="email" type="email" value={form.email} onChange={update} required className="mt-2 w-full rounded-md border-zinc-300" /></label>
        <label className="mt-4 block text-sm font-bold text-zinc-800">Phone<input name="phone" type="tel" placeholder="+91 9876543210" value={form.phone} onChange={update} required className="mt-2 w-full rounded-md border-zinc-300" /></label>

        <PasswordInput label="Password" name="password" value={form.password} onChange={update} visible={showPassword} setVisible={setShowPassword} />
        <PasswordStrengthMeter password={form.password} />
        <PasswordInput label="Confirm Password" name="confirm_password" value={form.confirm_password} onChange={update} visible={showConfirm} setVisible={setShowConfirm} />
...
function PasswordInput({ label, name, value, onChange, visible, setVisible }) {
  return (
