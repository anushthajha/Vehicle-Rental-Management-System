  ...
  return (
    <AuthLayout compact>
      <div className="w-full max-w-md rounded-lg border border-zinc-200 bg-white p-8 text-center shadow-sm">
        {status === 'loading' && (
          <>
            <Loader2 className="mx-auto animate-spin text-sigfleet" size={52} />
        ...
        {status === 'success' && (
          <>
            ...
            <p className="mt-3 text-zinc-600">You're ready to explore SigFleet.</p>
            <Link className="mt-6 inline-flex min-h-11 items-center justify-center rounded-md bg-sigfleet px-5 font-bold text-white" to="/auth/login">Login</Link>
          </>
        )}
        {status === 'failed' && (
          <>
            ...
            <form onSubmit={resend} className="mt-6 text-left">
              ...
              <button className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-md bg-sigfleet px-4 font-bold text-white" type="submit">Request new link</button>
            </form>
