...
export default function PaymentPage() {
  ...
  async function confirmPay() {
    ...
    sessionStorage.setItem('zoomcar_last_booking_success', JSON.stringify({ ...booking, transaction_id: response.data.transaction_id }))
      ...
      <section className="mx-auto max-w-3xl space-y-5">
        <div className="rounded-lg border border-zinc-200 bg-white p-5">
          ...
          <div className="mt-4 flex gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-2xl font-black text-zoomcar">₹{formatMoney(total)}</p>
            </div>
          </div>
        </div>
        ...
        {booking.payment?.status === 'paid' ? (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-6 text-center">
            ...
            <button onClick={() => navigate(`/booking/success?ref=${booking.booking_ref}`)} className="mt-4 rounded-md bg-zoomcar px-5 py-3 font-black text-white">View Confirmation</button>
          </div>
        ) : (
          <div className="rounded-lg border border-zinc-200 bg-white p-5">
            ...
            <button disabled={method === 'wallet' && !walletOk} onClick={() => { setDialogState('confirm'); setDialogOpen(true) }} className="mt-5 w-full rounded-md bg-zoomcar px-5 py-3 font-black text-white disabled:bg-zinc-300">Pay ₹{formatMoney(total)}</button>
          </div>
        )}
      </section>
      ...
      <Dialog.Root open={dialogOpen} onOpenChange={setDialogOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-32px)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-2xl">
            ...
            {dialogState === 'confirm' && <div className="mt-4 space-y-2 font-bold text-zinc-700"><p>Amount: ₹{formatMoney(total)}</p><p>Booking: {booking.booking_ref}</p><p>Car: {booking.car?.title}</p><p>Method: {method === 'wallet' ? 'Wallet' : 'Simulated Card Payment'}</p><p className="text-sm text-zinc-500">(Safe demo environment)</p><div className="mt-5 flex justify-end gap-3"><button onClick={() => setDialogOpen(false)} className="rounded-md border border-zinc-300 px-4 py-2 font-black">Cancel</button><button onClick={confirmPay} className="rounded-md bg-zoomcar px-4 py-2 font-black text-white">Confirm & Pay ₹{formatMoney(total)}</button></div></div>}
            {dialogState === 'processing' && <div className="grid min-h-40 place-items-center"><div className="text-center"><Loader2 className="mx-auto animate-spin text-zoomcar" /><p className="mt-3 font-black text-zinc-700">Processing payment...</p></div></div>}
            {dialogState === 'success' && <div className="mt-4 text-center"><CheckCircle2 className="mx-auto text-emerald-600" size={56} /><p className="mt-3 font-bold text-zinc-700">Transaction ID: {result?.transaction_id}</p><p className="font-bold text-zinc-700">Amount: ₹{formatMoney(total)} paid</p><button onClick={() => navigate(`/booking/success?ref=${result?.booking_ref || booking.booking_ref}`)} className="mt-5 rounded-md bg-zoomcar px-5 py-3 font-black text-white">View Booking Details</button></div>}
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
      ...
      <Dialog.Root open={addOpen} onOpenChange={setAddOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-32px)] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-5">
            ...
            <button onClick={addMoney} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md bg-zoomcar px-4 py-3 font-black text-white"><Plus size={17} /> Add to wallet</button>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
    </main>
...
function MethodCard({ active, onClick, icon: Icon, title, subtitle }) {
  return <button onClick={onClick} className={`rounded-md border p-4 text-left ${active ? 'border-zoomcar bg-red-50' : 'border-zinc-200'}`}><Icon className="text-zoomcar" /><p className="mt-3 font-black text-zinc-950">{title}</p><p className="mt-1 text-sm font-bold text-zinc-500">{subtitle}</p></button>
}
