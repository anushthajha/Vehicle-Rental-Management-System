    ...
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-zinc-50 px-4 py-10">
      ...
      <section className="relative w-full max-w-xl rounded-lg border border-zinc-200 bg-white p-8 text-center shadow-xl">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 180 }}>
          {isPending ? <Clock className="mx-auto text-amber-500" size={82} /> : <CheckCircle2 className="mx-auto text-emerald-600" size={82} />}
        </motion.div>
        <h1 className="mt-5 text-4xl font-black text-zinc-950">{isPending ? 'Booking Pending' : 'Booking Confirmed!'}</h1>
        ...
        <div className="mt-5 space-y-1 text-sm font-bold text-zinc-600">
          ...
          {summary.total_amount && <p>{isPending ? 'Amount due after approval' : 'Total paid'}: ₹{formatMoney(summary.total_amount)}</p>}
        </div>
        <p className="mt-5 text-sm font-bold text-zinc-500">
          {isPending
            ? `Your request was sent to the vehicle manager. You will be notified of the confirmation within 24 hours at ${user?.email || 'your email'}.`
