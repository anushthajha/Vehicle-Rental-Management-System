...
export default function CityPage() {
  ...
  useEffect(() => { api.get('/cars', { params: { city: name, limit: 12 } }).then((r) => setCars(r.data.cars || r.data.items || [])).catch(() => setCars([])) }, [name])
  const chips = areas[city] || ['Central', 'Airport Road', 'Business District', 'Railway Station']
  return <PublicShell><PageHero eyebrow={name} title={`Self-drive cars in ${name}`} subtitle={`Book verified cars across popular ${name} neighborhoods for city drives, business trips, and weekend escapes.`} image={cityImages[city]} /><Section title={`Popular areas in ${name}`}><div className="flex flex-wrap gap-2">{chips.map((area) => <Link key={area} to={`/vehicles?city=${name}&area=${area}`} className="rounded-full bg-white px-4 py-2 font-black shadow-sm">{area}</Link>)}</div></Section><Section title={`Cars available in ${name}`}><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{cars.map((car) => <VehicleCard key={car.id} car={car} />)}{!cars.length && <div className="rounded-lg border border-dashed border-zinc-300 p-10 text-center font-black text-zinc-500">No cars found yet.</div>}</div></Section><Section title="Travel tips"><div className="grid gap-4 md:grid-cols-3">{['Book morning pickups to avoid traffic peaks.', 'Keep toll and parking receipts until trip closure.', 'Choose SUV or MUV categories for longer group routes.'].map((tip) => <div key={tip} className="rounded-lg bg-white p-5 font-bold shadow-sm">{tip}</div>)}</div></Section><Section title={`Popular routes from ${name}`}><div className="grid gap-3 md:grid-cols-4">{['Weekend hills', 'Airport run', 'Beach escape', 'Heritage trail'].map((route) => <div key={route} className="rounded-lg border border-zinc-200 bg-white p-4 font-black">{name} → {route}</div>)}</div></Section></PublicShell>
}
