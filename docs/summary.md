...
Replaced the placeholder seed with a full MySQL + MongoDB demo dataset for the Zoomcar clone.

Implemented:
- `backend/app/seed.py` now creates the admin account, 5 approved hosts, 10 guests with varied KYC states, wallets, host profiles, and KYC records.
...
- Seeded booking lifecycle data covering completed, active, confirmed, pending, cancelled, rejected, and extension-request scenarios, with simulated payments, coupon usage, wallet transactions, refunds, and host earnings.
